...
def do_transfer(src, dst, amount, memo):
    ...
    if src_acct == None or dst_acct == None or src_acct == dst_acct:
    ...
    sql = [
        ('INSERT INTO xacts (xact_acct, xact_memo, xact_amount) VALUES (?, ?, ?)', [src, memo, -amount]),
        ('INSERT INTO xacts (xact_acct, xact_memo, xact_amount) VALUES (?, ?, ?)', [dst, memo, amount]),
        ('UPDATE accts SET acct_balance = ? WHERE acct_id = ?', [(src_acct.balance - amount), src]),
        ('UPDATE accts SET acct_balance = ? WHERE acct_id = ?', [(dst_acct.balance + amount), dst]),
    ]
